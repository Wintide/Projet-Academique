...
class Bomber:
    ...
    def to_dict(self):
        return {'position': self.position.to_dict(), 'niveau': self.niveau, 'pv': self.pv,
...
class Fantôme:
    ...
    def to_dict(self):
        return {'position': self.position.to_dict(),
                'case prec': self.case_précédente.to_dict() if self.case_précédente else None}
...
class Bombe:
    ...
    def to_dict(self):
        return {'position': self.position.to_dict(), 'timer': self.timer, 'portée': self.portée,
...
class Game:
    ...
    def __init__(self, carte: Map, pos_bombers: list[Position], pos_ethernets: list[Position], timerglobal: int,
                 timerfantôme: int):
    ...
    def affiche_carte(self):
        # Crée une copie de la carte pour l'affichage
        carte_temp = [list(ligne) for ligne in self.carte]

        # Place les fantômes sur la copie de la carte
        for fantome in self.fantômes:
            pos = fantome.position
            carte_temp[pos.y][pos.x] = 'F'

        # Affiche la carte mise à jour
        for ligne in carte_temp:
            print("".join(ligne))
    ...
    def deplacement_bomber(self, num_joueur: int, direction: Action):
        ...
        if self.est_non_bloquant(new):
            self.carte[position.y][position.x] = ' '
    ...
    def update_éliminations(self):
        ...
        if self.fantômes_éliminés or any(f.pv <= 0 for f in self.fantômes):
    ...
    def explose(self, b: Bombe) -> set[Bombe]:
        ...
        # Mise à jour : suppression des fantômes éliminés
        self.fantômes = [f for f in self.fantômes if f.pv > 0]
    ...
    def phase_non_joueur(self):
        ...
        # Mise à jour des éliminations
        self.update_éliminations()
